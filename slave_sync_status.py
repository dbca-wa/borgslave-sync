...
class SlaveSyncStatus(object):
    ...
    def __init__(self,sync_file,action = "update",file_content = None):
        if file_content:
            ...
            m = hashlib.md5()
            m.update(file_content.encode() if isinstance(file_content,str) else file_content)
    ...
    def is_not_succeed_except_preview_tasks(self):
        return any([s.is_not_succeed  for k,s in self._info.get("tasks",{}).items() if k not in ("get_layer_preview","send_layer_preview","delete_dumpfile")])
